...
class MarkovRNN(nn.Module):
    ...
    def __init_cells(self):
        ...
        for k in range(self.num_state):
            cell = RNNCell(input_size=self.input_dim, hidden_size=self.hidden_dim, bias=self.bias[0],
    ...
    def forward(self, x, h, pi):
        ...
        out_k = TimeDistributed(self.c)(h_k.permute(0, 2, 1)).permute(0, 2, 1)  # B x Ny x K
    ...
    def fit(self, data, verbose=True):
        ...
        start_hidden = self.__init_hidden(batch_size)
        start_belief = self.__init_belief(batch_size)
        self.hidden_state = start_hidden
        self.belief_state = start_belief
        ...
        for i in tqdm(range(1, len(data) + 1)):  # data pass
            ...
            start_idx = max(i - self.trunc_len, 0)
            inputs, targets = zip(*data[start_idx: i])
            ...
            def closure():
                ...
                for step, inp in enumerate(inputs):  # bptt pass
                    ...
                    pred, hidden, pred_k, hidden_k = self.forward(inp, hidden_list[-1], belief_list[-1])
    ...
    def predict(self, data, run_states=True):
        ...
        for step, inp in enumerate(inputs):  # bptt pass
            ...
            inp = inp.to(self.device)
            pred, hidden, pred_k, hidden_k = self.forward(inp, hidden, belief)
            pred_tensor_list.append(pred)
            target = targets[step].to(self.device)
